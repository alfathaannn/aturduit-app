...
class Store {
    ...
    addPocket(name, initialColor = 'blue') {
        const pocket = {
            id: generateId(),
            name,
            balance: 0,
    ...
    allocateToPocket(pocketId, amount) {
        ...
        if (this.state.mainBalance < amount) throw new Error('Saldo utama tidak cukup');
        ...
        const transaction = {
            ...
            type: 'expense', // Considered expense from main balance, or specialized type? 
                             // User asked for "recorded in history". 
                             // If we label it 'expense' it reduces stats. 
                             // But wait, "Spending" usually means money leaving the system.
                             // However, for pure tracking, let's use a special type 'transfer' or just 'expense' but filter stats?
                             // User requirement: "tercatat di riwayat transaksi".
                             // Let's use 'transfer' type if possible, OR 'expense' but maybe exclude from total expense stats?
                             // actually, let's keep it simple: It appears in history. 
                             // If I make it 'expense' it might confuse the "Total Pengeluaran". 
                             // Let's use 'transfer' type. ui.js renderHistory handles 'income' and 'expense'.
                             // We need to update ui.js to handle 'transfer' or just treat it visually distinct.
                             // For now opacity/color?
            type: 'transfer_out', 
            amount: parseFloat(amount),
            pocketId: pocketId, // Related pocket
            recurring: false
        };
    ...
    getTotalBalance() {
        const pocketsTotal = this.state.pockets.reduce((acc, p) => acc + p.balance, 0);
    ...
    importData(jsonData) {
        try {
            ...
            // Basic validation
            if (!parsed.pockets || !parsed.transactions) throw new Error('Invalid JSON format');
            this.state = parsed;
            this.saveState();
            return true;
        } catch (e) {
